...
## Steps
1. **Take one source.** Prefer an existing `knowledge/insights/` file. If it's a raw call, run
...
5. **Draft each touch.** Fill the matching `library/email-templates/` skeleton in Sid's voice
   (borrow `consulting-content-drafter`'s AIDA + "you over I"). Ground every claim in the source —
   no invented numbers, streams, or chart positions.
...
7. **Report the fan-out** (1 source → N drafts) with proposed send_dates spread across the week.
...
Source: Ch. 3/5 (atomization + the multi-format test) + `docs/plans/2026-06-19-email-engine-design.md`.
